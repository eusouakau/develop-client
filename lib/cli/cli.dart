...
class UsersCLI {
  ...
  Future<bool> menu() async {
    ...
    var options = [
      'Login',
      'Create user',
      'Auth User (internal, without confirm email)',
      'Update user',
      'Delete user',
      'List user',
      'Configurations',
    ...
    if (cli == options[0]) {
      ...
      //auto confirm
      await optionAutoConfirm();
    } else if (cli == options[3]) {
      //forgot pass
      await optionForgotPass();
  ...
  void optionAutoConfirm() async {
    clear();
    try {
      askEmail();
      var response = await _usersWebService.autoConfirm(_email);
      _response = 'response: ${response.body}';
    } on Exception {
      _response = 'Connection refused';
    }
  ...
  void optionForgotPass() async {
    clear();
    try {
      askEmail();
      var response = await _usersWebService.forgotPass(_email);
      _response = 'response: ${response.body}';
    } on Exception {
      _response = 'Connection refused';
    }
